observeEvent(input$dmaup, {
  ...
  dmareportmap <-
    fitBounds(
      dma_react$sasdma,
      min(dma_react$dmacoord$`Lon (Decimal Degrees)`) + 0.5,
      min(dma_react$dmacoord$`Lat (Decimal Degrees)`) - 0.5,
  ...
  for (i in 1:nrow(dmainfoinsert)) {
    dmainfovalues <-
      paste0(apply(dmainfoinsert[i, ], 1, function(x)
  ...
  dmacoordinsert <-
    left_join(dmainfo, dma_react$dmacoord, by = c("OLDID" = "ID")) %>%
  ...
  enable("dmareport")
  if ("i" %in% dma_react$dmanameout$INITOREXT) {
    enable("kml")
  }
  enable("dmaletter")
  ...
  output$dmareport <- downloadHandler(
    filename = paste0(
      ...
      date_formats$day1,
      " ",
      "Potential SLOW Zone_Report.pdf"
    ),
    ...
    content = function(file) {
      ...
      params <-
        list(
          ...
          dmanameout = dma_react$dmanameout,
          dmacoord = dma_react$dmacoord,
  ...
  letterbounds <-
    left_join(dma_react$alldmas, dma_react$dmacoord, by = "ID")
  print(letterbounds)
  #initial/extension
  ...
  if (1 %in% letterbounds$ID) {
    ...
    print(typeof(title1))
    NLat1 <-
      unique(DMA1$`Lat (Degree Minutes)`[DMA1$`Lat (Decimal Degrees)` == max(DMA1$`Lat (Decimal Degrees)`)])
    SLat1 <-
      unique(DMA1$`Lat (Degree Minutes)`[DMA1$`Lat (Decimal Degrees)` == min(DMA1$`Lat (Decimal Degrees)`)])
    WLon1 <-
      unique(DMA1$`Lon (Degree Minutes)`[DMA1$`Lon (Decimal Degrees)` == max(DMA1$`Lon (Decimal Degrees)`)])
    ELon1 <-
      unique(DMA1$`Lon (Degree Minutes)`[DMA1$`Lon (Decimal Degrees)` == min(DMA1$`Lon (Decimal Degrees)`)])
  ...
  if (2 %in% letterbounds$ID) {
    ...
    print(typeof(title2))
    NLat2 <-
      unique(DMA2$`Lat (Degree Minutes)`[DMA2$`Lat (Decimal Degrees)` == max(DMA2$`Lat (Decimal Degrees)`)])
    SLat2 <-
      unique(DMA2$`Lat (Degree Minutes)`[DMA2$`Lat (Decimal Degrees)` == min(DMA2$`Lat (Decimal Degrees)`)])
    WLon2 <-
      unique(DMA2$`Lon (Degree Minutes)`[DMA2$`Lon (Decimal Degrees)` == max(DMA2$`Lon (Decimal Degrees)`)])
    ELon2 <-
      unique(DMA2$`Lon (Degree Minutes)`[DMA2$`Lon (Decimal Degrees)` == min(DMA2$`Lon (Decimal Degrees)`)])
  ...
  if (3 %in% letterbounds$ID) {
    ...
    NLat3 <-
      unique(DMA3$`Lat (Degree Minutes)`[DMA3$`Lat (Decimal Degrees)` == max(DMA3$`Lat (Decimal Degrees)`)])
    SLat3 <-
      unique(DMA3$`Lat (Degree Minutes)`[DMA3$`Lat (Decimal Degrees)` == min(DMA3$`Lat (Decimal Degrees)`)])
    WLon3 <-
      unique(DMA3$`Lon (Degree Minutes)`[DMA3$`Lon (Decimal Degrees)` == max(DMA3$`Lon (Decimal Degrees)`)])
    ELon3 <-
      unique(DMA3$`Lon (Degree Minutes)`[DMA3$`Lon (Decimal Degrees)` == min(DMA3$`Lon (Decimal Degrees)`)])
  ...
  if (4 %in% letterbounds$ID) {
    ...
    NLat4 <-
      unique(DMA4$`Lat (Degree Minutes)`[DMA4$`Lat (Decimal Degrees)` == max(DMA4$`Lat (Decimal Degrees)`)])
    SLat4 <-
      unique(DMA4$`Lat (Degree Minutes)`[DMA4$`Lat (Decimal Degrees)` == min(DMA4$`Lat (Decimal Degrees)`)])
    WLon4 <-
      unique(DMA4$`Lon (Degree Minutes)`[DMA4$`Lon (Decimal Degrees)` == max(DMA4$`Lon (Decimal Degrees)`)])
    ELon4 <-
      unique(DMA4$`Lon (Degree Minutes)`[DMA4$`Lon (Decimal Degrees)` == min(DMA4$`Lon (Decimal Degrees)`)])
    ...
    content = function(file) {
      ...
      if (criteria$triggrptrue == TRUE) {
        if (criteria$DMAapp == "acoudet") {
          acou_obs <- dma_react$egsas %>%
            ...
            mutate(URL = replace(
              URL,
              grepl('Woods Ho', INSTITUTION) == TRUE,
      ...
      params <-
        list(
          ...
          date1 = date_formats$date1,
          triggerdateletter = triggerdateletter,
          triggerword = triggerword,
          letterdirect = letterdirect,
          landmark = landmark,
          observer = observer,
          neworextlet = neworextlet,
          title1 = title1,
          NLat1 = NLat1,
          SLat1 = SLat1,
          WLon1 = WLon1,
